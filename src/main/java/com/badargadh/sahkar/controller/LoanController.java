...
public class LoanController extends BaseController implements Initializable {
    ...
    private SortedList<LoanApplication> sortedData;
    ...
    public void handleGlobalShortcuts(ShortcutKeyEvent event) {
        ...
        if (vboxLoanForm != null && vboxLoanForm.getScene() != null && vboxLoanForm.isVisible()) {
        	Platform.runLater(() -> {
                if (event.getCommand().equals("SAVE")) {
                	AppLogger.info("Loan Application Module: Received SAVE Event");
                    handleApply();
                }
                else if (event.getCommand().equals("RESET")) {
                	AppLogger.info("Loan Application Module: Received RESET Event");
                    resetForm();
                }
            });
        }
    ...
    public void initialize(URL location, ResourceBundle resources) {
    	...
    	txtApplicationNo.setOnKeyPressed(event -> {
    		if (event.getCode() == KeyCode.ENTER && txtApplicationNo.getText() != null && !txtApplicationNo.getText().isEmpty()) {
    ...
    public void loadDynamicData(String type) {
    	List<LoanApplication> summaries = new ArrayList<LoanApplication>();
    	if(type.equals("RECENT")) {
    		summaries.addAll(loanService.getRecentApplications(LocalDateTime.now().minusMonths(4)));
    	}
    	else if(type.equals("ALL_DATA")) {
    		summaries.addAll(loanService.getAllApplications());
    	}
    	loanData.setAll(summaries);
    	
    	FilteredList<LoanApplication> filteredData = new FilteredList<LoanApplication>(loanData, p -> true);
    ...
    @FXML
    private void handleRejectApplications() {
    	financialMonthService.getActiveMonth().ifPresent(month -> {
    		List<LoanApplication> applications = loanService.getAllApplicationsWithStatuses(month, 
    				Arrays.asList(LoanApplicationStatus.APPLIED, LoanApplicationStatus.WAITING));
    		System.err.println("Total size :: "+applications.size());
        	if(DialogManager.confirm("Do you really want to reject all the pending applications? ",
        			"Use this button only if you want to reject all the remaining applications for current month only")) {
        		if(showSecurityGate("Please confirm your identity to reject all the applications")) {
        			applications.stream().forEach(app -> {
        				loanService.rejectApplication(app, LoanApplicationStatus.REJECTED, "Not Selected");
        				System.err.println("Loan rejected :: "+app.getMember().getMiddleNameGuj());
        			});
        		}
        	}
    	});
